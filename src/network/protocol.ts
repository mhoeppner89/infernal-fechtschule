...
import type { ActorSnapshot, GameSnapshot, InputFrame, ItemSnapshot, Lane, LessonId, SceneryId } from '../sim/types.js';
 ...
 */
export const PEER_PROTOCOL_VERSION = 3 as const;
...
const SCENERY = new Set<SceneryId>(['cobbled-streets', 'town-gate', 'sala-darmi', 'castello']);
...
function isSnapshot(value: unknown): value is GameSnapshot {
  ...
  if (typeof value.waveTitle !== 'string' || value.waveTitle.length > 160) return false;
  if (typeof value.waveLabel !== 'string' || value.waveLabel.length > 64) return false;
  ...
  if (!isIntegerInRange(value.levelIndex, 0, 8) || typeof value.levelName !== 'string' || value.levelName.length > 64) return false;
  if (value.scenery !== null && !SCENERY.has(value.scenery as SceneryId)) return false;
  if (!isIntegerInRange(value.waveInLevel, 0, 64) || !isIntegerInRange(value.wavesInLevel, 1, 64)) return false;
  if (!isFiniteNumber(value.score) || !isIntegerInRange(value.bossPhase, 0, 16)) return false;
  if (!isFiniteNumber(value.cameraX) || !isFiniteNumber(value.roadWidth) || value.roadWidth <= 0) return false;
  ...
  if (!isLane(value.lane)) return false;
  // The open doorway is level state the guest draws, so it has to travel.
  if (typeof value.exitOpen !== 'boolean') return false;
  // And so is the stand clock, which the guest's HUD counts down.
  if (!isFiniteNumber(value.holdRemaining) || value.holdRemaining < 0) return false;
  ...
  if (!Array.isArray(value.items) || value.items.length > 64) return false;
  return value.actors.every(isActorSnapshot) && value.items.every(isItemSnapshot);
}

function isLane(value: unknown): value is Lane | null {
...
function isItemSnapshot(value: unknown): value is ItemSnapshot {
  if (!isRecord(value)) return false;
