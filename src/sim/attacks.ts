...
/** Every archetype the tempo table covers: the field cast, minus the boss. */
export type FieldArchetype = Exclude<Archetype, 'meyer' | 'grotesque'>;

/**
 * What makes one archetype feel different from another is cadence, not stats.
 * Each field archetype declares its own decision rate, its own swing window,
 * and the interrupt it owns, so "thugs press in pairs", "the captain answers"
 * and "the spear line punishes crowding" are data rather than folklore.
 */
export interface EnemyTempo {
  /**
   * Pause between the end of one swing and the next commitment, so the rhythm
   * is the time from the end of a move rather than from the moment it started
   * (a slow attack must not eat its own recovery).
   */
  readonly cooldown: readonly [number, number];
  /** Cadence of its non-attacking decisions (guard reads, repositioning). */
  readonly think: readonly [number, number];
  /** Forward distance it works at while committed. */
  readonly station: number;
  /** Forward distance it mills at while it has no attack permission. */
  readonly waitStation: number;
  /** Forward band it commits a swing inside. */
  readonly band: readonly [number, number];
  /** Maximum depth offset it will swing across. */
  readonly depth: number;
  /** A second forward band that gets a different answer (the spear's butt). */
  readonly braceBand?: readonly [number, number];
  /** Thugs: the mate that swings on a beat, so a press comes from two clubs. */
  readonly pair?: {
    readonly radius: number;
    readonly beatDelay: number;
    readonly beatRecovery: number;
    /** How far out a mate may still accept the beat and step into it. */
    readonly beatForward: number;
    readonly beatDepth: number;
  };
  /** The spear line: what it does when the target is buried in a crowd. */
  readonly line?: {
    readonly crowdRadius: number;
    readonly crowdCount: number;
    /** Station it holds while it has a turn, and where it closes on a crowd. */
    readonly commitStation: number;
    readonly crowdStation: number;
    readonly crowdCooldown: readonly [number, number];
  };
  /** Plate: the cut an armoured archetype throws back at a shrugged-off light. */
  readonly answer?: {
    readonly attack: string;
    /** Beat between the shrug and the answer, so the player can read it. */
    readonly delay: number;
    /** How long the answer stays armed if the player leaves reach. */
    readonly window: number;
    /** Lockout, so poking plate is punished once per exchange. */
...
} as const satisfies Record<FieldArchetype, EnemyTempo>;

export const ENEMY_TEMPO: Readonly<Record<FieldArchetype, EnemyTempo>> = Object.freeze(FIELD_TEMPO);
 ...
 * his learned routes.
 */
const KIT_STARTERS: Readonly<Record<Weapon, { light: string; heavy: string; counter: string; dodge: string; crouch: string; air: string; entry: string }>> = Object.freeze({
...
export function throwAttackFor(weapon: Weapon): string | null {
  if (weapon === 'club') return 'cl_throw';
  if (weapon === 'spear') return 'sp_throw';
  return null;
}
...
const ATTACK_LESSON = new Map<string, LessonId>();
for (const [lesson, attacks] of Object.entries(LESSON_ATTACKS)) {
  for (const id of attacks) ATTACK_LESSON.set(id, lesson as LessonId);
}

export function isAttackUnlocked(attackId: string, lessons: ReadonlySet<LessonId>): boolean {
  if (BASIC_ATTACKS.has(attackId)) return true;
