import type { Archetype, ItemKind, Lane, SceneryId } from './types.js';

/**
 * A group of arrivals: what walks in, how many, and how it is spread out.
 */
export interface SpawnSpec {
  archetype: Exclude<Archetype, 'meyer'>;
  count: number;
  /**
   * Seconds between the members of this group. Short intervals are how a wave
   * arrives as a *clump* rather than a queue: measured, seven thugs at 0.4 s
   * intervals cost a competent bot nothing at all, because a crowd never more
   * than one deep is met one at a time.
   */
  interval: number;
  /**
   * Player frontier that unlocks this group, in the level's road coordinates.
   * Defaults to an even slice of the road, which is right for a crowd that walks
   * in behind the march; a wave whose idea lives on a particular piece of ground
   * authors it, so the fight happens where the wave says it does rather than
   * wherever the player happened to still have enemies alive.
   */
  from?: number;
  /**
   * The edge this group walks in from. Left to the sim's own alternation by
   * default — a crowd is more interesting arriving from both flanks — and
   * authored when the *side* is part of the encounter's meaning: the bound thing
   * comes up the hall behind the party rather than ahead of them.
   */
  side?: 'east' | 'west';
}

/** The one idea a wave exists to teach. */
export type EncounterKind = 'press' | 'choke' | 'ambush' | 'flood' | 'duel' | 'hold' | 'boss';
...
/** What the road has lying on it when the wave opens. */
...
interface WaveBase {
  /**
   * The encounter's name *inside its level* — what this stretch of the place is
   * about. The level's own name is the place; this is the fight, and the banner
   * announces the two together (THE TOWN · WAVE 1 OF 1 · STREET RABBLE).
   */
  title: string;
  subtitle: string;
  /** Offer a lesson card once this wave is cleared. */
  lessonAfter?: boolean;
  /**
   * Little Fighter 2's stages are furnished as well as populated, and a draught
   * on the bench is the honest answer to a long attrition fight: the player has
   * to cross the ground for it, in the middle of the fight, instead of the enemy
   * being made weaker.
   */
  provisions?: readonly Provision[];
  /**
   * A narrowing of the road (`Lane`), which the shape of the ground makes a
   * cheaper way to make a fight than any number in it. A choke requires one —
   * everyone filing through the same narrow ground *is* its idea.
   */
  lane?: Lane;
}

/** The crowd: groups that walk in behind the player's march. */
export interface PressWave extends WaveBase {
  kind: 'press';
  groups: readonly SpawnSpec[];
}

/** The gate: everyone has to come through the same narrow ground. */
export interface ChokeWave extends WaveBase {
  kind: 'choke';
  groups: readonly SpawnSpec[];
  lane: Lane;
}

/** The trap: a front to fix the player, then a group at their back. */
export interface AmbushWave extends WaveBase {
  kind: 'ambush';
  groups: readonly SpawnSpec[];
  ambush: {
    /** Player frontier that the trap waits for, in the level's road coordinates. */
    trigger: number;
    /**
     * How few of the bait line may still stand before the doors open. The trap
     * is sprung by the bait *breaking*, not by a mark on the road: sprung into
     * the middle of the bait it was four more bodies in an ongoing fight (60
     * mean, 110 worst of a 110 bar), and sprung after the bait was dead it
     * delivered three wretches into a standing player's back for 0 damage in
     * every seed measured. A player who simply runs the street is still caught,
     * by the frontier check in the sim rather than by this one.
     */
    remaining: number;
    /** Arrives behind the player. */
    behind: readonly SpawnSpec[];
  };
}

/** The flood: the road empties, then fills, on a clock the player reads. */
export interface FloodWave extends WaveBase {
  kind: 'flood';
  surges: {
    /** Seconds between one surge and the next (the lull is the reward). */
    every: number;
    /** Each surge is its own set of arrivals; the flank alternates. */
    waves: readonly (readonly SpawnSpec[])[];
  };
}

/** The duel: one opponent, no crowd, and no way to make it a crowd. */
export interface DuelWave extends WaveBase {
  kind: 'duel';
  duel: {
    archetype: Exclude<Archetype, 'meyer'>;
    /**
     * Which end of the level's road the champion holds. He is placed off the
     * screen's edge when the level opens (the party walks in at the other end),
     * so the duel is something the player walks toward.
     */
    end: 'east' | 'west';
    z: number;
  };
...
/** The stand: outlast the clock while the road keeps feeding. */
export interface HoldWave extends WaveBase {
  kind: 'hold';
  hold: {
    /** Seconds to outlast. Spawning stops when it reaches zero. */
    seconds: number;
    /** Seconds between beats. */
    beats: readonly (readonly SpawnSpec[])[];
    every: number;
  };
}

/** The finale: the bound thing, and whatever it drags in with it. */
export interface BossWave extends WaveBase {
  kind: 'boss';
  groups: readonly SpawnSpec[];
}
...
/**
 * A level: one place, one dressing, one road, and the waves fought along it.
 *
 * This is the campaign's structural unit — what Little Fighter 2 calls a stage.
 * The party walks a level's road from west to east, enemies arrive in *waves*
 * along it (always from off the screen's edge, never out of thin air in front of
 * the player), and the eastern doorway only opens once the level's last wave is
 * down. Crossing that doorway is what ends the level; clearing a wave inside one
 * only brings the next one on.
 */
export interface LevelDefinition {
  readonly name: string;
  readonly setting: SceneryId;
  /**
   * Full walkable width of the level's road in world pixels. One road per level,
   * walked once: the waves inside it are fought further along the same ground,
   * so a wave boundary cannot teleport anybody.
   *
   * It also has to hide a spawn: `MIN_LEVEL_ROAD` is the width at which an
   * arrival can always find off-screen road no matter where the player stands,
   * and `tests/level-structure.test.mjs` holds every level to it.
   */
 ...
 */
export const ENCOUNTER_LABELS: Readonly<Record<EncounterKind, string>> = Object.freeze({
  press: 'THE PRESS',
 ...
 */
export function laneNarrowingAt(x: number, lane: Lane | null | undefined): number {
  if (!lane) return 0;
  if (x >= lane.from && x <= lane.to) return 1;
 ...
 */
const CAMPAIGN: readonly LevelDefinition[] = [
  {
    name: 'The Town',
...
export const LEVELS: readonly LevelDefinition[] = deepFreeze(CAMPAIGN);
