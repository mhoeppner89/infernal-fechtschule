...
const TOUCH_BUTTONS: readonly TouchButton[] = ['light', 'heavy', 'mobility', 'guard', 'switch'];
...
export class TouchControls {
  ...
  private readonly buttonPointerOwners = new Map<number, TouchButton>();
  private readonly buttonPointers = new Map<TouchButton, Set<number>>();
  private readonly buttonElements = new Map<TouchButton, Set<HTMLElement>>();
  ...
  private joystickDeadZone = 0;

  constructor(
    root: HTMLElement,
    onPressed: (button: TouchButton) => void = () => undefined,
  ...
  get hasMovementPointer(): boolean { return this.joystickPointer !== null; }
  ...
  reset(): void {
    ...
    for (const elements of this.buttonElements.values()) {
      for (const element of elements) element.classList.remove('is-held');
    }
  ...
  private bindButtons(root: HTMLElement): void {
    for (const element of root.querySelectorAll<HTMLElement>('[data-button]')) {
      ...
      if (!button || !TOUCH_BUTTONS.includes(button)) continue;

      const elements = this.buttonElements.get(button) ?? new Set<HTMLElement>();
      elements.add(element);
      this.buttonElements.set(button, elements);
      if (!this.buttonPointers.has(button)) this.buttonPointers.set(button, new Set<number>());
      ...
      const release = (event: PointerEvent): void => {
        if (!this.buttonPointerOwners.has(event.pointerId)) return;
        event.preventDefault();
        this.releaseButtonPointer(event.pointerId);
      };
  ...
  private bindLifecycle(): void {
    if (typeof window !== 'undefined' && typeof window.addEventListener === 'function') {
      ...
      window.addEventListener('resize', () => {
        if (this.joystickPointer !== null) this.reset();
      });
    }
  ...
  private updateJoystick(clientX: number, clientY: number): void {
    ...
    const magnitude = Math.min(
      1,
      (length - this.joystickDeadZone) / Math.max(1, this.joystickRadius - this.joystickDeadZone)
    );
  ...
  private updateButtonVisual(button: TouchButton): void {
    ...
    if (isHeld) this.held.add(button);
    else this.held.delete(button);
    for (const element of this.buttonElements.get(button) ?? []) {
      if (isHeld) element.classList.add('is-held');
      else element.classList.remove('is-held');
    }
  }
