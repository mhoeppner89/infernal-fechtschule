import type { ActorSnapshot, ActorState, Archetype, Weapon } from '../sim/types.js';
import { ATTACKS } from '../sim/attacks.js';
import {
  ANIMATION_MANIFEST,
  ANIMATION_TICKS_PER_SECOND,
  animationClipId,
  type AnimationClipSpec,
  type AnimationFrameSpec,
  type ReadyAnimationClip
} from './animation-manifest.js';

export type AnimationAssetStatus = 'not-started' | 'loading' | 'loaded' | 'failed' | 'unsupported';
...
export interface ResolvedAnimationFrame {
  readonly clip: ReadyAnimationClip;
  readonly frame: AnimationFrameSpec;
  readonly frameIndex: number;
  readonly image: HTMLImageElement;
  /** Shared per-clip correction that keeps body scale consistent across states. */
  readonly scaleCorrection: number;
}

/**
 * A deliberately short transition from the previous authored pose. The new
 * pose is always drawn at full opacity; only this faint previous-pose trail is
 * faded out. That keeps the presentation responsive instead of delaying a key
 * fighting-game silhouette with a conventional cross-fade.
 */
...
const NO_ANIMATION_TRANSITION: AnimationTransitionSpec = Object.freeze({
  duration: 0,
  previousOpacity: 0
});
const REACTION_STATES: ReadonlySet<ActorState> = new Set(['hitstun', 'guardbreak', 'dead']);
const LOCOMOTION_STATES: ReadonlySet<ActorState> = new Set(['idle', 'move', 'block']);

export function sameAnimationFrame(
  left: ResolvedAnimationFrame,
  right: ResolvedAnimationFrame
): boolean {
 ...
 */
export function animationTransitionSpec(
  previous: ResolvedAnimationFrame,
  next: ResolvedAnimationFrame
  ...
  if (next.frame.cue === 'contact') return NO_ANIMATION_TRANSITION;
...
export interface AnimationReadinessFailure {
  readonly url: string;
  readonly error: string;
}

export interface AnimationReadinessReport {
  readonly declaredClips: number;
  readonly readyClips: number;
  readonly plannedClips: number;
  readonly declaredAssets: number;
  readonly loadedAssets: number;
  readonly pendingAssets: number;
  readonly failedAssets: number;
  readonly unsupportedAssets: number;
  readonly normalizedClips: number;
  readonly unnormalizedClipIds: readonly string[];
  readonly minScaleCorrection: number;
  readonly maxScaleCorrection: number;
  readonly failures: readonly AnimationReadinessFailure[];
}
 ...
 */
const SCALE_CORRECTION_OVERRIDES: Readonly<Record<string, number>> = Object.freeze({
...
function upperQuartile(values: readonly number[]): number {
  if (values.length === 0) return 0;
  const sorted = [...values].sort((left, right) => left - right);
...
export function characterInkMass(pixels: Uint8ClampedArray): number {
  let mass = 0;
  for (let index = 0; index + 3 < pixels.length; index += 4) {
    ...
    if (luminance > 205 && saturation < 0.16) continue;
    mass += alpha;
  }
  return mass;
}
...
function positiveHold(frame: AnimationFrameSpec): number {
  return Math.max(1, Math.floor(frame.holdTicks));
}

function clipTick(clip: ReadyAnimationClip, stateElapsed: number, stateDuration: number): number {
  ...
  if (duration > 0) {
    if (elapsed >= duration) return totalTicks - 1;
...
function frameFromCueGroup(
  clip: ReadyAnimationClip,
  indices: readonly number[],
  phaseProgress: number
): number {
  if (indices.length === 0) return 0;
  const totalTicks = indices.reduce(
    (sum, index) => sum + positiveHold(clip.frames[index] ?? clip.frames[0]),
    0
  );
  let remaining = Math.min(
    totalTicks - 1,
    Math.floor(Math.max(0, Math.min(0.999999, phaseProgress)) * totalTicks)
  );
...
export function attackAnimationFrameIndex(
  clip: ReadyAnimationClip,
  attackId: string,
  elapsed: number
): number {
  const definition = ATTACKS[attackId];
  if (!definition) return animationFrameIndex(clip, elapsed, 0);
  ...
  const recovery = clip.frames
    .map((frame, index) => (
      ...
      || (frame.cue === 'neutral' && index > finalContactIndex)
    ) ? index : -1)
    .filter((index) => index >= 0);

  if (elapsed < definition.startup) {
    return frameFromCueGroup(
      clip,
      anticipation,
      definition.startup > 0 ? elapsed / definition.startup : 1
    );
  }
  ...
  if (elapsed < activeEnd) {
    return frameFromCueGroup(
      clip,
      contact,
      definition.active > 0 ? (elapsed - definition.startup) / definition.active : 1
    );
  }
  return frameFromCueGroup(
    clip,
    recovery,
    definition.recovery > 0 ? (elapsed - activeEnd) / definition.recovery : 1
  );
}

function actorLookup(actor: ActorSnapshot): AnimationLookup {
...
export function resolveAttackClipId(attackId: string | null): string | null {
  if (!attackId) return null;
  return ATTACKS[attackId]?.animation ?? attackId;
}

export class SpriteAnimationCatalog {
  private readonly clips = new Map<string, AnimationClipSpec>();
  private readonly assets = new Map<string, AssetRecord>();
  private readonly clipScaleCorrections = new Map<string, number>();

  constructor(manifest: readonly AnimationClipSpec[] = ANIMATION_MANIFEST) {
    for (const clip of manifest) {
      if (this.clips.has(clip.id)) throw new Error(`Duplicate animation clip id: ${clip.id}`);
      this.clips.set(clip.id, clip);
      if (clip.availability === 'ready') {
        if (clip.display.fixedScale) this.clipScaleCorrections.set(clip.id, 1);
  ...
  lookup(query: AnimationLookup): AnimationClipSpec | null {
  ...
  lookupActor(actor: ActorSnapshot): AnimationClipSpec | null {
    const query = actorLookup(actor);
    const preferred = this.lookup(query);
    if (preferred || !query.variant) return preferred;
   ...
   */
  resolveFrameByIndex(clip: ReadyAnimationClip, frameIndex: number): ResolvedAnimationFrame | null {
    const frame = clip.frames[frameIndex];
    if (!frame) return null;
    const asset = this.assets.get(frame.url);
    if (!asset || asset.status !== 'loaded' || !asset.image) return null;
    return {
      clip,
      frame,
      frameIndex,
  ...
  resolveActorFrame(actor: ActorSnapshot): ResolvedAnimationFrame | null {
    const clip = this.lookupActor(actor);
    if (!clip || clip.availability !== 'ready') return null;
  ...
  getReadiness(): AnimationReadinessReport {
    ...
    return {
      ...
      pendingAssets: assets.filter(
        (asset) => asset.status === 'not-started' || asset.status === 'loading'
      ).length,
  ...
  private finishAssetLoad(asset: AssetRecord, image: HTMLImageElement): void {
    ...
    asset.status = 'loaded';
    asset.error = null;
    if (asset.inkMass === null) asset.inkMass = this.measureInkMass(image);
    this.recalibrateScaleCorrections();
  }

  private measureInkMass(image: HTMLImageElement): number | null {
  ...
  private recalibrateScaleCorrections(): void {
    const ready = [...this.clips.values()].filter(
      (clip): clip is ReadyAnimationClip => clip.availability === 'ready'
    );
    const referenceMasses = new Map<Archetype, number[]>();
    for (const clip of ready) {
      if (clip.state !== 'idle') continue;
      const masses = clip.frames
        .map((frame) => this.assets.get(frame.url)?.inkMass ?? null)
        .filter((mass): mass is number => mass !== null && mass > 0);
      if (masses.length !== clip.frames.length) continue;
      const group = referenceMasses.get(clip.archetype) ?? [];
      group.push(...masses);
      referenceMasses.set(clip.archetype, group);
    }

    const targets = new Map<Archetype, number>();
    ...
    for (const clip of ready) {
      ...
      const correction = override === undefined
        ? Math.min(
          ...
          Math.max(MIN_SCALE_CORRECTION, rawCorrection)
        )
        : Math.min(
          MAX_EXPLICIT_SCALE_CORRECTION,
          Math.max(MIN_SCALE_CORRECTION, override)
        );
