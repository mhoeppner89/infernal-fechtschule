...
import type { ActorSnapshot, GameEvent, GameSnapshot } from '../sim/types.js';
...
export class CanvasRenderer {
  ...
  handle(event: GameEvent): void {
    ...
    if (event.type === 'guardbreak' || event.type === 'boss-phase') this.shake = Math.max(this.shake, 15);
    if (event.type === 'death') {
  ...
  render(snapshot: GameSnapshot, dt: number): void {
    ...
    for (const actor of sorted) this.drawActor(actor);
    this.drawParticles();
