...
import type { ActorSnapshot, GameEvent, GameSnapshot, HitZone } from '../sim/types.js';
import {
...
import { BACKGROUND_MANIFEST, BackgroundCatalog, type BackgroundReadinessReport } from './background-catalog.js';
...
export class CanvasRenderer {
  ...
  private backgroundWave = -1;
  private previousBackgroundWave = -1;
  ...
  render(snapshot: GameSnapshot, dt: number): void {
    ...
    for (const actor of drawOrder) this.drawActor(actor);
  ...
  private drawStageBounds(snapshot: GameSnapshot): void {
    const context = this.context;
  ...
  private drawBackgroundImage(image: HTMLImageElement, time: number, waveIndex: number, alpha: number): void {
    const context = this.context;
  ...
  private updateBackground(waveIndex: number, dt: number): void {
  ...
  private drawEnemyBar(actor: ActorSnapshot): void {
    const context = this.context;
