import { ENEMY_TEMPO, attackDuration, getAttack, isHitZoneExposed, isAttackActive, isImprovisedWeapon, resolveCrouchAttack, resolveDodgeAttack, resolveGuardAttack, resolvePlayerAttack, resolveSwitchAttack, throwAttackFor } from './attacks.js';
...
import { ageActionBuffer, FOLLOW_UP_BUFFER_SECONDS, guardDirectionFromInput, GUARD_COMMAND_WINDOW_SECONDS, MAX_PLAYER_CHAIN_LENGTH, writeActionBuffer } from './combo.js';
 ...
 */
export function laneDepthRange(x, lane) {
 ...
 */
const THROWN_ATTACKS = Object.freeze({
    club: Object.freeze({
 ...
 */
export const ITEM_REACH_STATES = new Set([
    'idle',
    'move',
    'crouch',
    'block',
 ...
 */
export function canTakeItem(bearer, item) {
    if (item.kind === 'potion')
        return bearer.health < bearer.maxHealth;
 ...
 */
export function itemWithinReach(actor, item) {
    if (item.y > ITEM_GRAB_HEIGHT)
        return false;
    if (Math.hypot(item.x - actor.x, item.z - actor.z) > ITEM_PICKUP_RADIUS + actor.radius)
        return false;
...
export class GameWorld {
    ...
    chooseLesson(id) {
        if (this.phase !== 'lesson' || !this.offeredLessons.includes(id))
            return false;
        this.lessons.add(id);
        this.offeredLessons = [];
    ...
    updateActorTimers(dt) {
        for (const actor of this.actors) {
            ...
            if (actor.guardCommand) {
                ...
                if (actor.guardCommand.elapsed > GUARD_COMMAND_WINDOW_SECONDS)
    ...
    updatePlayer(actor, input, dt) {
        ...
        if (input.guardPressed && actor.guard > 0 && ['idle', 'move', 'block', 'crouch'].includes(actor.state))
        ...
        if (input.switchPressed && this.reachForItem(actor))
            return;
        ...
        if (actor.state === 'dodge') {
            ...
            this.clampActor(actor);
            if (this.peekBufferedAction(actor) === 'light' &&
        ...
        if (actor.state === 'crouch') {
            ...
            actor.vx = 0;
            actor.vz = 0;
            if (this.peekBufferedAction(actor) !== null &&
        ...
        if (actor.state === 'block') {
            ...
            if (actor.guardCommand)
                actor.facing = actor.guardCommand.referenceFacing;
            return;
        }
        if (input.switchPressed) {
            this.startSwitch(actor, false);
     ...
     */
    updateGuardCommand(actor, input) {
        if (!['idle', 'move', 'block', 'crouch'].includes(actor.state))
            return;
        ...
        if (!input.guardHeld && !actor.guardCommand)
            return;
        const direction = guardDirectionFromInput(input.moveX, input.moveZ, actor.guardCommand?.referenceFacing ?? actor.facing);
        if (!direction)
            return;
        if (!actor.guardCommand) {
            if (!input.guardPressed)
    ...
    leaveGuardForMobility(actor, input) {
        ...
        if (input.lightPressed || input.heavyPressed || bufferedAction === 'light' || bufferedAction === 'heavy') {
            ...
            if (bufferedAction === 'light' || bufferedAction === 'heavy')
                this.consumeBufferedAction(actor);
            this.startAttack(actor, resolveCrouchAttack(actor.weapon, action));
            return;
        }
    ...
    startPlayerAction(actor, action, currentAttackId = null, chain = false) {
        ...
        const attackId = resolvePlayerAttack(actor.weapon, action, currentAttackId, afterParry, this.lessons);
        if (afterParry)
            actor.counterWindow = 0;
        actor.parryWindow = 0;
        actor.guardCommand = null;
    ...
    updatePlayerAttack(actor, input, dt) {
        ...
        this.captureActionEdges(actor, input, 'light-heavy');
        if (input.switchPressed && runtime.hitConfirmed)
            this.writeAction(actor, 'switch');
        runtime.elapsed += dt;
        ...
        if (runtime.elapsed < attackDuration(definition) && !(linkOpen && queuedAction !== null))
            return;
    ...
    updateEnemy(actor, dt) {
        ...
        const inBand = (band) => forward >= band[0] && forward <= band[1] && depth <= tempo.depth;
        ...
        if (actor.attackPermission && actor.aiCooldown <= 0 && actor.pairBeatTimer <= 0) {
            ...
            if (braces || inBand(tempo.band)) {
                ...
                if (attackId === 'thug_press')
                    this.armPairBeat(actor);
                return;
            }
        }
     ...
     */
    chooseFieldAttack(actor, target, forward, braced) {
        ...
        switch (actor.archetype) {
            ...
            case 'thug':
                ...
                if (forward > 60)
                    return roll < 0.6 ? 'thug_press' : 'thug_overhead';
                if (target.state === 'crouch')
                    return roll < 0.58 ? 'thug_low' : 'thug_body';
    ...
    isCrowded(target, tempo) {
        ...
        for (const enemy of this.actors) {
            if (enemy.team !== 'enemies' || enemy.state === 'dead')
                continue;
            if (this.distanceSquared(enemy, target) > limit)
                continue;
            count += 1;
            if (count >= line.crowdCount)
                return true;
        }
        return false;
    }
    ...
    armPairBeat(actor) {
        ...
        for (const other of this.actors) {
            if (other === actor || other.archetype !== 'thug' || other.state === 'dead')
                continue;
            if (other.pairBeatTimer > 0)
                continue;
            if (other.state === 'hitstun' || other.state === 'guardbreak')
                continue;
            ...
            if (other.state === 'attack' && other.attack) {
                ...
                if (other.attack.elapsed < swing.startup + swing.active)
                    continue;
            }
     ...
     */
    restAfterSwing(actor, target) {
        if (actor.archetype === 'meyer' || actor.archetype === 'grotesque')
            return 0;
        const tempo = ENEMY_TEMPO[actor.archetype];
        const line = tempo.line;
    ...
    updateItems(dt) {
        for (let index = this.items.length - 1; index >= 0; index -= 1) {
            ...
            if (item.age >= ITEM_LIFETIME_SECONDS)
     ...
     */
    resolveThrownHit(item) {
        const kind = item.kind;
        if (kind !== 'club' && kind !== 'spear')
            return;
        const thrower = this.actors.find((actor) => actor.id === item.thrownBy);
        if (!thrower || thrower.state === 'dead')
            return;
        const definition = THROWN_ATTACKS[kind];
        for (const target of this.actors) {
            if (target.team === thrower.team || target.state === 'dead')
                continue;
            if (target.invulnerable > 0 || item.hitIds.has(target.id))
                continue;
            if (Math.hypot(target.x - item.x, target.z - item.z) > target.radius + 18)
                continue;
            ...
            if (this.tryBlock(thrower, target, definition))
                this.deflectItem(item, target, definition);
            else
                this.applyHit(thrower, target, definition);
            if (!previous)
                thrower.attack = null;
            item.vx *= 0.3;
            return;
    ...
    deflectItem(item, target, definition) {
   ...
   */
    reachForItem(actor) {
        if (!ITEM_REACH_STATES.has(actor.state))
            return false;
        let nearest = null;
        let nearestDistance = Number.POSITIVE_INFINITY;
        for (const item of this.items) {
            if (!itemWithinReach(actor, item))
                continue;
    ...
    chipCarriedWeapon(attacker) {
        if (attacker.team !== 'players' || !isImprovisedWeapon(attacker.weapon))
            return;
    ...
    tryParryOrDeflect(attacker, target, definition) {
        ...
        if (!deflected)
            return false;
        if (timedParry)
            target.parryWindow = 0; // One timed parry per deliberate Guard edge.
    ...
    applyHit(attacker, target, definition) {
        ...
        if (attacker.team === 'players' && definition.heavy && attacker.openingTimer > 0) {
            multiplier = 1.52;
            if (attacker.weapon === 'longsword' && this.lessons.has('ls-provoker'))
                multiplier = 1.78;
        ...
        if (attacker.team === 'players') {
            ...
            if (definition.id === 'ls_l2' && this.lessons.has('ls-crossing'))
                damage *= 1.1;
            if (definition.id === 'ds_l2' && this.lessons.has('ds-backhand'))
                damage *= 1.1;
            if (definition.id === 'ls_l3' && this.lessons.has('ls-threefold'))
                damage *= 1.12;
            if (definition.id === 'ds_l3' && this.lessons.has('ds-wheel'))
                damage *= 1.1;
        }
        ...
        if (shruggedOff && target.health > 0) {
            ...
            if (target.archetype !== 'meyer' && target.archetype !== 'grotesque') {
                ...
                if (answer)
                    target.answerTimer = Math.max(target.answerTimer, answer.window);
            }
        }
        else {
            this.clearActionBuffer(target);
    ...
    startAttack(actor, attackId, explicitTarget, options = {}) {
    ...
    bufferPlayerActionsDuringHitStop(inputs) {
        for (let index = 0; index < this.playerCount; index += 1) {
    ...
    captureActionEdges(actor, input, mode) {
        if (mode === 'light') {
            if (input.lightPressed)
                this.writeAction(actor, 'light');
            return;
        }
        ...
        if (input.lightPressed)
            ...
            this.writeAction(actor, 'heavy');
    }
    writeAction(actor, action) {
    ...
    startDodgeFromLightRecovery(actor, input) {
    ...
    bufferPlayerEdge(playerIndex, source) {
        ...
        if (stored)
            this.pendingPlayerEdgeAges[playerIndex] = 0;
    }
    ...
    updateSpawns(dt) {
        ...
        const definition = this.wave;
        if (definition) {
            if (definition.kind === 'flood')
                this.updateSurgeClock(definition);
            else if (definition.kind === 'hold')
                this.updateHoldClock(definition, dt);
            else if (definition.kind === 'ambush')
                this.checkAmbushTrigger(definition);
        }
    ...
    updateHoldClock(definition, dt) {
        ...
        if (!beat || elapsed < this.holdBeat * definition.hold.every)
            return;
        const standing = this.enemyActors().filter((enemy) => enemy.state !== 'dead').length;
        if (standing > HOLD_CROWD)
            return;
        this.holdBeat += 1;
        this.enqueueBeat(beat, this.spawnClock);
     ...
     */
    checkAmbushTrigger(definition) {
        if (this.ambushSprung)
            return;
        ...
        if (!reachedTrigger)
            return;
        if (!baitIsBroken && frontier < definition.ambush.trigger + AMBUSH_PURSUIT)
            return;
        ...
        if (behindX === null && frontier < definition.ambush.trigger + AMBUSH_PURSUIT)
            return;
    ...
    drainSpawnQueue() {
        ...
        while (schedule.length > 0) {
            ...
            if (!entry || entry.at > this.spawnClock)
                break;
            ...
            if (entry.threshold > Number.NEGATIVE_INFINITY &&
            ...
            if (arrival && entry.urgency)
                arrival.urgency = entry.urgency;
     ...
     * whole read.
     */
    spawnDuel(definition) {
        if (this.duelSpawned)
            return;
        this.duelSpawned = true;
        let frontier = ARENA.minX;
    ...
    updateCamera() {
        ...
        if (excess <= 0)
            return;
        this.cameraX = clamp(this.cameraX + Math.sign(offset) * excess, minCameraX, maxCameraX);
    }
    /**
    ...
    beginWave(index) {
        ...
        if (definition.kind === 'duel')
            this.spawnDuel(definition);
     ...
     */
    enqueueBeat(groups, at = this.spawnClock, together = false) {
        let cursor = at;
    ...
    spawnEnemy(archetype, arrival = {}) {
        ...
        let side = arrival.side ?? (this.nextActorId % 2 === 0 ? 'east' : 'west');
        ...
        if (x === null)
            x = this.furthestRoadEnd(frontier);
    ...
    encounterIsSpent() {
        ...
        if (!definition)
            return true;
        if (definition.kind === 'flood')
            return this.surgeIndex >= definition.surges.waves.length;
        if (definition.kind === 'hold')
            return this.holdRemaining <= 0;
        if (definition.kind === 'ambush')
            return this.ambushSprung;
        return true;
    }
    /**
    ...
    checkWaveResolution(dt) {
        if (this.waveResolved || !this.encounterIsSpent())
            return;
        ...
        if (!this.isLastWaveOfLevel()) {
            this.clearTimer += dt;
            if (this.clearTimer < WAVE_CLEAR_DELAY)
                return;
            this.clearTimer = 0;
