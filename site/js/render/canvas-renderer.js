...
export class CanvasRenderer {
    ...
    drawBackground(snapshot) {
        ...
        if (current.length > 0 || previous.length > 0) {
            ...
            this.drawBackgroundLayerSet(this.backgroundScenery, 'back', previous.length > 0 ? eased : 1);
            return;
        }
        const context = this.context;
    ...
    drawBackgroundForeground() {
        ...
        const eased = 1 - Math.pow(1 - this.backgroundTransition, 3);
        this.drawBackgroundLayerSet(this.backgroundScenery, 'front', previous.length > 0 ? eased : 1);
    }
    drawBackgroundLayerSet(scenery, order, alpha) {
        if (!scenery || alpha <= 0)
            return;
    ...
    drawBackgroundLayer(layer, alpha) {
        const context = this.context;
