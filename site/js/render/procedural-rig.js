...
const BODY_HEIGHT = Object.freeze({
    meyer: 176,
    thug: 166,
    spear: 178,
    captain: 186,
...
const HEAD_RADIUS = Object.freeze({
    meyer: 16,
    thug: 15,
    spear: 15,
    captain: 17,
...
const WEAPON_BLADE_LENGTH = Object.freeze({
    longsword: 104,
    dussack: 78,
    club: 58,
    spear: 132
});
const WEAPON_GRIP_LENGTH = Object.freeze({
    longsword: 19,
    dussack: 9,
    club: 8,
    spear: 25
...
export function addPoint(left, right) {
    return point(left.x + right.x, left.y + right.y);
}
export function subtractPoint(left, right) {
    return point(left.x - right.x, left.y - right.y);
}
export function scalePoint(value, scale) {
    return point(value.x * finiteOr(scale, 0), value.y * finiteOr(scale, 0));
}
export function distance(left, right) {
    return Math.hypot(right.x - left.x, right.y - left.y);
}
export function lerpPoint(left, right, amount) {
    const t = clamp01(amount);
    return point(left.x + (right.x - left.x) * t, left.y + (right.y - left.y) * t);
}
export function rotatePoint(value, angle) {
 ...
 */
export function solveTwoBoneIK(root, target, upperLength, lowerLength, bendDirection = 1) {
    ...
    const cosine = clamp((upper * upper + clampedDistance * clampedDistance - lower * lower)
        / (2 * upper * clampedDistance), -1, 1);
    const bend = direction + Math.acos(cosine) * (bendDirection < 0 ? -1 : 1);
    const elbow = point(root.x + Math.cos(bend) * upper, root.y + Math.sin(bend) * upper);
    const end = point(root.x + Math.cos(direction) * clampedDistance, root.y + Math.sin(direction) * clampedDistance);
    return { root: point(root.x, root.y), elbow, end, upperLength: upper, lowerLength: lower };
}
export function bladeLengthFor(weapon) {
    return WEAPON_BLADE_LENGTH[weapon];
}
export function gripLengthFor(weapon) {
    return WEAPON_GRIP_LENGTH[weapon];
}
export function actorBodyHeight(archetype) {
    return BODY_HEIGHT[archetype];
}
export function actorHeadRadius(archetype) {
...
function attackStyle(actor, definition) {
    const id = actor.attackId ?? '';
    const weapon = definition?.weapon ?? actor.weapon;
    if (weapon === 'spear' || id.includes('thrust') || id.includes('brace'))
        return 'thrust';
    if (id.includes('sweep') || id.includes('shock') || id.includes('leap'))
        return 'sweep';
    ...
    if (id.includes('l2') || id.includes('backhand') || id.includes('follow'))
        return 'cross';
    if (id.includes('low'))
        return 'rising';
    return hashString(id || actor.archetype) > 0.52 ? 'diagonal' : 'cross';
}
function attackAngle(style, progress, facing) {
...
export function attackMotionFor(actor) {
    ...
    const phase = actor.state !== 'attack'
        ? 'idle'
...
function crouchAmount(actor, attack) {
    if (actor.state === 'crouch')
        return 0.94;
    if (attack.definition?.crouchedPosture)
        return 0.82;
    if (actor.state === 'block')
        return 0.12;
    if (actor.state === 'dodge')
        return 0.56;
    if (attack.style === 'rising')
        return 0.6;
    return 0;
}
function motionStride(actor, attack, time) {
    if (actor.state === 'move')
        return Math.sin(finiteOr(actor.stateElapsed, 0) * 10.5 + actor.id * 0.27);
    if (actor.state === 'dodge')
        return Math.sin(clamp01(actor.stateElapsed / Math.max(EPSILON, actor.stateDuration)) * Math.PI);
    if (actor.state === 'attack')
        return attack.phase === 'contact' ? 0.72 : attack.recoil * -0.26;
    return Math.sin(time * 2.05 + actor.id * 0.31) * 0.08;
}
function weaponAngle(actor, attack) {
    if (actor.state === 'block')
        return -1.2;
    ...
    if (actor.weapon === 'spear')
        return -0.05;
    if (actor.weapon === 'club')
        return -0.35;
    return -0.42;
}
function bladeFromGrip(grip, weapon, angle, bladeLength = bladeLengthFor(weapon)) {
...
function bladeTrailFor(blade, attack, actor) {
    if (actor.state !== 'attack' || attack.phase === 'anticipation' || attack.style === 'thrust')
        return [];
    const points = [];
    ...
    for (let index = count; index >= 0; index -= 1) {
        ...
        points.push(point(blade.base.x + direction.x * length, blade.base.y + direction.y * length));
    }
    return points;
}
function jumpAmount(actor, attack) {
    if (actor.state === 'jump') {
        const progress = clamp01(actor.stateElapsed / Math.max(EPSILON, actor.stateDuration));
        return Math.sin(progress * Math.PI) * 66;
    }
    if (actor.attackId?.includes('_air_'))
        return Math.sin(attack.progress * Math.PI) * 54;
    if (attack.style === 'sweep' && actor.attackId?.includes('leap'))
        return Math.sin(attack.progress * Math.PI) * 30;
    return 0;
}
function bodyRotationFor(actor, attack) {
    if (actor.state === 'dead') {
        return -0.04 + clamp01(actor.stateElapsed / Math.max(EPSILON, actor.stateDuration)) * 1.18;
    }
    if (actor.state === 'hitstun')
        return actor.reactionZone === 'legs' ? -0.18 : -0.3;
    ...
    if (actor.state === 'attack') {
        if (attack.style === 'thrust')
            return 0.08;
        if (attack.phase === 'anticipation')
            return -0.1;
        if (attack.phase === 'contact')
            return 0.1;
        return -0.05;
    }
    return 0;
 ...
 */
export function poseForActor(actor, time = 0) {
    ...
    const frontHand = point(grip.x + gripDirection.x * gripLength * 0.5, grip.y + gripDirection.y * gripLength * 0.5);
    const rearHand = twoHanded
        ? point(grip.x - gripDirection.x * gripLength * 0.5, grip.y - gripDirection.y * gripLength * 0.5)
...
export function hitZoneAnchor(actor, zone) {
    const pose = poseForActor(actor, actor.stateElapsed);
    if (zone === 'head')
        return pose.head;
    if (zone === 'legs')
        return lerpPoint(pose.frontKnee, pose.frontFoot, 0.55);
    return pose.chest;
}
...
//# sourceMappingURL=procedural-rig.js.map