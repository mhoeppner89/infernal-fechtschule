...
function drawPath(context, points, close = true) {
...
function drawArchShape(context, x, y, width, height) {
...
function drawMasonry(context, x, y, width, height, seed, base, joint, rowHeight = 12) {
...
function drawPlasterDetails(context, x, y, width, height, seed, shadow, highlight) {
    context.save();
...
function roundedWindow(context, x, y, width, height, glow) {
    context.save();
...
function drawRoof(context, x, baseY, width, height, color, seed = 0) {
...
function drawVoussoirs(context, centerX, springY, radiusX, radiusY, count, color, seed) {
...
function drawArchOpening(context, x, y, width, height, stone, opening, glow, seed) {
    drawArchShape(context, x, y, width, height);
...
function drawDoor(context, x, y, width, height, wood, stone, seed) {
...
function drawArcade(context, x, y, width, height, count, stone, stoneLight, opening, seed) {
    if (count <= 0)
        return;
    ...
    for (let index = 0; index < count; index += 1) {
        ...
        drawArchOpening(context, archX, y, archWidth, height, index % 2 === 0 ? stone : stoneLight, opening, index % 3 === 0 ? 0.16 : 0.05, seed + index * 7);
...
function drawTimberFrame(context, x, y, width, height, wood, highlight, seed) {
    context.save();
    context.lineJoin = 'round';
...
function drawBalcony(context, x, y, width, palette, seed) {
...
function drawClothAwning(context, x, y, width, depth, color, stripe, seed) {
    drawPath(context, [
        { x, y },
...
function drawHangingSign(context, x, y, width, color, accent, seed) {
...
function drawChimney(context, x, top, width, height, palette, seed) {
...
function drawChurchTower(context, x, baseY, width, height, palette, seed, clock = false) {
    ...
    drawArchOpening(context, x + width * 0.2, top + height * 0.24, width * 0.6, height * 0.34, palette.masonryLight, '#20353e', 0.2, seed + 2);
    if (clock) {
...
function drawDistantRoofscape(context, offset, palette, seed, towerX, towerHeight, clock = false) {
...
function drawRenaissanceBuilding(context, spec, palette) {
    ...
    context.fill();
    if (spec.stone) {
        drawMasonry(context, x + width + 1, sideTop + 2, side - 2, sideBottom - sideTop - 3, seed + 18, spec.sideTone, 'rgba(27,45,50,0.28)', 11);
    }
    else {
    ...
    for (let window = 0; window < windowCount; window += 1) {
        ...
        roundedWindow(context, wx, wy, windowWidth, 35 + (window % 2) * 5, 0.35 + noise(seed + window * 9, 22) * 0.5);
    }
    if (spec.timber)
        drawTimberFrame(context, x + 2, top + 4, width - 4, bottom - top - 10, palette.wood, palette.woodLight, seed + 28);
    if (spec.arcade) {
        drawArcade(context, x + 18, top + 91, width - 36, Math.max(54, bottom - top - 83), spec.arcade, palette.masonryDark, palette.masonryLight, '#263d45', seed + 38);
    }
    if (spec.balcony)
        drawBalcony(context, x + width * 0.18, top + 76, width * 0.64, palette, seed + 48);
    if (spec.awning) {
        drawClothAwning(context, x + width * 0.1, top + 71, width * 0.8, 24, palette.accent, palette.accentAlt, seed + 55);
    }
    if (spec.sign)
        drawHangingSign(context, x + width * 0.78, top + 77, 34, palette.accentAlt, palette.accent, seed + 64);
    if (spec.door) {
        drawDoor(context, x + width * (0.44 + noise(seed, 73) * 0.12), bottom - 73, 30, 70, palette.wood, palette.masonryLight, seed + 80);
    }
    if (spec.battlements) {
        context.fillStyle = spec.facade;
...
function drawHangingCloth(context, x, y, width, height, color, stripe, sway) {
    context.save();
...
function drawGatehouseLandmark(context, x, baseY, width, palette, seed) {
    ...
    for (const towerX of [x, x + width - towerWidth]) {
        drawMasonry(context, towerX, towerTop, towerWidth, baseY - towerTop, seed + Math.round(towerX), palette.masonry, 'rgba(24,41,47,0.34)', 11);
        context.fillStyle = palette.masonryLight;
        ...
        drawArchOpening(context, towerX + towerWidth * 0.25, towerTop + 27, towerWidth * 0.5, 37, palette.masonryLight, '#1c3039', 0.12, seed + merlonSeed(towerX));
    }
    context.fillStyle = palette.masonry;
    context.fillRect(x + towerWidth - 3, wallTop, width - towerWidth * 2 + 6, baseY - wallTop);
    drawMasonry(context, x + towerWidth, wallTop + 3, width - towerWidth * 2, baseY - wallTop - 3, seed + 9, palette.masonry, 'rgba(23,41,47,0.32)', 12);
    context.fillStyle = palette.masonryLight;
    ...
    drawArchOpening(context, x + width * 0.32, wallTop + 38, width * 0.36, baseY - wallTop - 35, palette.masonryLight, '#162a34', 0.12, seed + 12);
    context.strokeStyle = palette.woodLight;
    context.lineWidth = 2;
    context.beginPath();
...
function merlonSeed(value) {
    return Math.round(value * 0.37);
}
function drawKeepLandmark(context, x, baseY, width, palette, seed) {
    ...
    drawMasonry(context, x + towerWidth * 0.5, wallTop, width - towerWidth, baseY - wallTop, seed + 1, palette.masonry, 'rgba(22,39,45,0.34)', 11);
    for (const towerX of [x, x + width - towerWidth]) {
        drawMasonry(context, towerX, towerTop, towerWidth, baseY - towerTop, seed + Math.round(towerX), palette.masonryLight, 'rgba(24,40,45,0.38)', 10);
        context.fillStyle = palette.masonryDark;
        ...
        drawArchOpening(context, towerX + towerWidth * 0.2, towerTop + 33, towerWidth * 0.6, 46, palette.masonryLight, '#172c36', 0.16, seed + 20 + Math.round(towerX));
    }
    context.fillStyle = palette.masonryLight;
    ...
    drawArcade(context, x + towerWidth * 0.7, wallTop + 31, width - towerWidth * 1.45, baseY - wallTop - 30, 4, palette.masonryDark, palette.masonryLight, '#1b313b', seed + 30);
    context.fillStyle = palette.accent;
    drawPath(context, [
...
function drawMarketStall(context, x, y, width, palette, seed, scene) {
...
function drawWeaponRack(context, x, y, palette, seed) {
...
function drawTorch(context, x, y, palette, seed, time) {
...
function drawStreetFoundation(context, offset, palette, seed) {
...
export class ProceduralSceneRenderer {
    ...
    drawSky(context, scene, time, bossPhase) {
    ...
    drawStaticPlane(context, scene, cameraX, alpha) {
        if (alpha <= 0)
            return;
    ...
    surfaceFor(scene) {
        ...
        if (existing)
            return existing;
        if (this.failedSurfaces.has(scene))
            return null;
        if (this.surfaces.size >= MAX_SURFACE_COUNT)
            return null;
        try {
            let surface = null;
    ...
    drawArchitecture(context, scene, offset) {
        if (scene === 'town-gate')
            this.drawTownGate(context, offset);
        else if (scene === 'sala-darmi')
            this.drawSalaDarmi(context, offset);
        else if (scene === 'castello')
            this.drawCastello(context, offset);
        else
            this.drawCobbledStreet(context, offset);
    }
    drawCobbledStreet(context, offset) {
        ...
        for (let index = 0; index < 7; index += 1) {
            ...
            drawHangingCloth(context, x, 116 + (index % 3) * 8, 34 + (index % 2) * 9, 52 + (index % 3) * 10, index % 2 === 0 ? palette.accent : palette.accentAlt, palette.plasterWarm, index % 3 - 1);
        }
        drawStreetFoundation(context, offset, palette, 121);
    }
    drawTownGate(context, offset) {
        ...
        for (let index = 0; index < 5; index += 1) {
            ...
            drawHangingCloth(context, x, 106 + (index % 2) * 8, 42, 70, index % 2 === 0 ? palette.accent : palette.accentAlt, palette.masonryLight, index - 2);
        }
        drawStreetFoundation(context, offset, palette, 319);
    }
    drawSalaDarmi(context, offset) {
        ...
        for (let index = 0; index < 6; index += 1) {
            ...
            context.fill();
            drawHangingCloth(context, x, 99, 35 + (index % 2) * 8, 58 + (index % 3) * 8, index % 2 === 0 ? palette.accent : palette.accentAlt, palette.plasterWarm, 0);
            if (index % 2 === 0)
                drawWeaponRack(context, x - 33, 189, palette, index + 12);
        }
        drawStreetFoundation(context, offset, palette, 491);
    }
    drawCastello(context, offset) {
        ...
        for (let index = 0; index < 5; index += 1) {
            ...
            drawHangingCloth(context, x, 82 + (index % 2) * 9, 42, 73, index % 2 === 0 ? palette.accent : palette.accentAlt, palette.masonryLight, index);
        }
        drawStreetFoundation(context, offset, palette, 751);
    }
    drawRoad(context, snapshot, cameraX) {
    ...
    drawCobblePerspective(context, minX, maxX, cameraX) {
        ...
        while (y < ROAD_BOTTOM + 24) {
            ...
            for (let col = first; col <= last; col++) {
                ...
                context.beginPath();
                context.moveTo(left + 2, y + jitter + 1);
                context.lineTo(right - 3, y - jitter + 1);
                context.lineTo(right, y + 3);
                context.lineTo(right - 1, bottom - 2);
                context.lineTo(right - 4, bottom);
                context.lineTo(left + 2, bottom + jitter * .5);
                context.lineTo(left, bottom - 3);
                context.lineTo(left - .3, y + 4);
                context.closePath();
                const tone = Math.round(39 + n * 25 - depth * 7);
                context.fillStyle = `rgb(${tone + 4},${tone + 20},${tone + 25})`;
                context.fill();
                context.strokeStyle = 'rgba(13,28,33,.58)';
                context.lineWidth = .8;
                context.stroke();
                context.strokeStyle = n > .55 ? 'rgba(198,190,158,.25)' : 'rgba(130,157,155,.16)';
                context.beginPath();
                context.moveTo(left + 3, y + 2);
                context.lineTo(right - 3, y + 2);
                context.stroke();
                if (n > .7 && depth > .15) {
                    context.strokeStyle = 'rgba(12,27,31,.23)';
                    context.beginPath();
                    context.moveTo(left + w * .36, y + 4);
                    context.lineTo(left + w * .43, y + h * .5);
                    context.lineTo(left + w * .3, bottom - 1);
                    context.stroke();
                }
            }
            y += h;
    ...
    drawForegroundStructure(context, scene, cameraX, time) {
        const parallax = 0.68;
        ...
        for (let index = -1; index < 7; index += 1) {
            ...
            if (screenX < -230 || screenX > this.width + 230)
                continue;
            if (screenX > 255 && screenX < this.width - 225)
                continue;
            const stallWidth = 142 + (index % 3) * 18;
            drawMarketStall(context, x, 211 + (index % 2) * 3, stallWidth, palette, index + 91, scene);
            drawHangingCloth(context, x + stallWidth * 0.54, 188, 29 + (index % 2) * 6, 44, index % 2 === 0 ? palette.accent : palette.accentAlt, palette.plasterWarm, Math.sin(time * 0.8 + index) * 1.5);
            if (scene === 'sala-darmi') {
    ...
    drawAtmosphere(context, scene, time, bossPhase, cameraX) {
        context.save();
        ...
        if (bossPhase >= 1) {
            ...
            for (let index = 0; index < 7; index += 1) {
                ...
                context.bezierCurveTo(this.width * 0.26, y - 80, this.width * 0.76, y + 100, index % 2 ? 0 : this.width, y + 24);
                context.stroke();
            }
            context.restore();
        }
...
//# sourceMappingURL=procedural-scene.js.map